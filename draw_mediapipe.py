...
def main(args, device, dataset_train, dataloader_train):
    ...
    for iter_, views_subset in tqdm(enumerate(dataloader_train)):
        ...
        gbuffer = renderer.render_batch(views_subset['camera'], deformed_vertices=deformed_vertices.contiguous(), deformed_normals=d_normals,
        ...
        for i in range(len(frame_name)):
            print(frame_name[i])
            output_mesh = args.run_name + '_' + frame_name[i] + '.png'
            ...
            print(os.path.join(output_dir, args.run_name + '_' + frame_name[i] + '_org.png'))
            imageio.imwrite(os.path.join(output_dir, args.run_name + '_' + frame_name[i] + '_org.png'), org_image.cpu().numpy())
