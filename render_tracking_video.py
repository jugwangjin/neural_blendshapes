...
def main(args, device):
    ...
    dataset_train2    = DatasetLoader(args2, train_dir=args.train_dir, sample_ratio=1, pre_load=False, train=True)
    dataset_train      = DatasetLoader(args2, train_dir=args.eval_dir, sample_ratio=1, pre_load=False)
    ...
    dataloader_train    = torch.utils.data.DataLoader(dataset_train, batch_size=1, collate_fn=dataset_train.collate, drop_last=True, shuffle=False)

    iteration = 0
    os.makedirs(output_dir, exist_ok=True)

    gt_writer = cv2.VideoWriter(f'{output_dir}/{args.run_name}_{args.video_name}_gt.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (512, 512))
    no_personalization_writer = cv2.VideoWriter(f'{output_dir}/{args.run_name}_{args.video_name}_no_personalization.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (512, 512))
    rgb_writer = cv2.VideoWriter(f'{output_dir}/{args.run_name}_{args.video_name}_rgb.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (512, 512))
    ...
    for iter_, views_subset in progress_bar:
        ...
        gt_image_converted = np.clip(np.rint(dataset_util.rgb_to_srgb(gt_iamge).detach().cpu().numpy() * 255.0), 0, 255).astype(np.uint8)
        for i in range(len(gt_iamge)):
            gt_writer.write(cv2.cvtColor(gt_image_converted[i], cv2.COLOR_RGB2BGR))
            # print(f"GT image shape: {gt_image_converted[i].shape}")
...
#             imageio.imsave(file_name.replace(f'.png', f'_normal_{i}.png'), convert_uint_255(normal))    
        ...
        pred_color_masked_converted = np.clip(np.rint(dataset_util.rgb_to_srgb(pred_color_masked).detach().cpu().numpy() * 255.0), 0, 255).astype(np.uint8)
        for i in range(len(pred_color_masked_converted)):
            rgb_writer.write(cv2.cvtColor(pred_color_masked_converted[i], cv2.COLOR_RGB2BGR))
        ...
        pred_color_masked_converted = np.clip(np.rint(dataset_util.rgb_to_srgb(pred_color_masked).detach().cpu().numpy() * 255.0), 0, 255).astype(np.uint8)
        for i in range(len(pred_color_masked_converted)):
            no_personalization_writer.write(cv2.cvtColor(pred_color_masked_converted[i], cv2.COLOR_RGB2BGR))

    gt_writer.release()
    rgb_writer.release()
    no_personalization_writer.release()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='bala', help='Name of the run in model_dir')
    parser.add_argument('--video_name', type=str, default='test', help='Name of the video in dataset')
    parser.add_argument('--output_dir_name', type=str, default='figures/tracking', help='Path to save the results. The results will be saved in model_dir / run_name / output_dir_name')
