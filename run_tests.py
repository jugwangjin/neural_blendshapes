...
def worker(gpu_id, command_queue):
    while not command_queue.empty():
        ...
        #         print(subd, os.listdir(os.path.join(OUTPUT_DIR_ROOT, subd)))
        print(directory, os.path.exists(os.path.join('/Bean/log/gwangjin/2024/nbshapes_comparisons/ours_enc_v14/', directory, 'stage_1', 'network_weights', 'neural_blendshapes_latest.pt')),
            os.path.exists(os.path.join('/Bean/log/gwangjin/2024/nbshapes_comparisons/ours_enc_v14/', directory, 'final_eval.txt')))
        if os.path.exists(os.path.join('/Bean/log/gwangjin/2024/nbshapes_comparisons/ours_enc_v14/', directory, 'stage_1', 'network_weights', 'neural_blendshapes_latest.pt')) \
            and not os.path.exists(os.path.join('/Bean/log/gwangjin/2024/nbshapes_comparisons/ours_enc_v14/', directory, 'final_eval.txt')):
...
# output_dir = /Bean/log/gwangjin/2024/neural_blendshapes
    ...
    for directory in directories:
        ...
        # if directory.startswith('nf_01') or directory.startswith('nf_03'):
        #     continue
        ...
        command = (f"CUDA_VISIBLE_DEVICES={{}} python test.py --config {conf_file} --output_dir {output_dir}", directory)
