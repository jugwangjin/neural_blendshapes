...
def main(args, device, dataset_train2, dataset_train):
    ...
    #     # also save the gt 
        ...
        features[:, [ict_facekit.expression_names.tolist().index(name) for name in smiles]] = features[:, [ict_facekit.expression_names.tolist().index(name) for name in smiles]].clamp(0.4)
