...
def main(args, device):
    ...
    for index in args.index:
        ...
        # whatever larger than 0.5 subtract 0.5
        # less than 0.5, keep 
        # facs_ = facs - 0.5
        # negative_facs_ = (facs_ < 0).float()

        # facs = facs * negative_facs_ + (facs - 0.5) * (1 - negative_facs_)

        # facs[:, [ict_facekit.expression_names.tolist().index(name) for name in happiness]] += 0.5
        facs_ = facs.clone()
        facs[:, [ict_facekit.expression_names.tolist().index(name) for name in jawopen]] += 0.5
        ...
        os.makedirs(output_dir, exist_ok=True)
        file_name = os.path.join(output_dir, f'{args.run_name}_{args.video_name}_{index}_exag.png')
        save_manipulation_image(pred_color_masked, views_subset, gbuffers["normal"], gbuffer_mask, file_name)

        facs = facs_
        facs[:, [ict_facekit.expression_names.tolist().index(name) for name in smiles]] *= 0.15
        ...
        os.makedirs(output_dir, exist_ok=True)
        file_name = os.path.join(output_dir, f'{args.run_name}_{args.video_name}_{index}_supp.png')
        save_manipulation_image(pred_color_masked, views_subset, gbuffers["normal"], gbuffer_mask, file_name)
