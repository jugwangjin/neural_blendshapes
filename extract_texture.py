...
def main(args, device):
    ...
    print(NeuralShader.__file__) 
    print(shader.custom_forward)
    shader = NeuralShader.load(os.path.join(args.output_dir, args.run_name, 'stage_1', 'network_weights', 'shader.pt'), device=device)
    shader.eval()
    output_dir = './debug/texture_map'
    os.makedirs(output_dir, exist_ok=True)

    position_map = create_position_map(ict_canonical_mesh, ict_facekit.uvs, ict_facekit.uv_faces, ict_facekit.canonical[0], f'debug/texture_map/{args.model_name}.png')
    ...
    print(ko.shape)
    # save as images
    cv2.imwrite(os.path.join(output_dir, "kd"), (kd * 255).astype(np.uint8))
    cv2.imwrite(os.path.join(output_dir, "kr"), (kd * 255).astype(np.uint8))
    cv2.imwrite(os.path.join(output_dir, "ko"), (kd * 255).astype(np.uint8))

if __name__ == '__main__':
    import configargparse
    parser = configargparse.ArgumentParser()
    parser.add_argument('--model_dir', type=str, default='/Bean/log/gwangjin/2024/nbshapes_comparisons/ours_enc_v3', help='Path to the trained model')
