...
class FCBottleneck(nn.Module):
    def __init__(self, in_features, out_features, bias=True, first_omega=30, hidden_omega=30.0):
        super().__init__()

        layers = []

        # First layer
        layer = FullyConnectedBlock(in_features, out_features, bias=bias, activation=None)
        layer.apply(lambda module: init_weights_normal(module=module, n=in_features))
        layers.append(layer)
