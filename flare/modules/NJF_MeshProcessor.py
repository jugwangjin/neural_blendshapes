...
import sys
sys.path.append('NeuralJacobianFields/source_njf')
...
class MeshProcessor:
    ...
    def get_centroids(self, use_wks=False):
        if self.centroids.points_and_normals is None:
            if not self.from_file:
                try:
                    self.load_centroids()
                except Exception as e:
                    self.compute_centroids(use_wks)
    ...
    def compute_centroids(self, use_wks=False):
