...
def ict_loss(ict_facekit, return_dict, views_subset, neural_blendshapes, renderer, lmk_adaptive, fullhead_template=False):
    ...
    ict_loss = torch.abs(ict_ * 10 - deformed_vertices * 10).reshape(ict_.shape[0], -1).mean(dim=-1)
    ...
    random_ict_loss = torch.abs(random_ict * 10 - random_deformed_vertices * 10).reshape(random_ict.shape[0], -1).mean(dim=-1)
