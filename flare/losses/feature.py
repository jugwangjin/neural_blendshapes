...
def feature_regularization_loss(feature, gt_facs, neural_blendshapes, bshape_modulation, views, mode, facs_weight=0, mult=1):
    facs = feature[..., :53]
    ...
    facs_reg = facs_reg.mean() * 1e1 * mult
    ...
    transform_origin_reg = torch.pow(transform_origin[0], 2).mean() + torch.pow(after_translation, 2).mean() 
