import torch
def feature_regularization_loss(feature, gt_facs, estim_lmks, gt_lmks, model_scale, iterations, facs_adaptive, facs_weight=0):
    facs = feature[..., :53]
    ...
    facs_regularization = torch.mean(torch.pow(facs+1e-2, 0.75))

    # latent regularization: rotation, translation to be zero, scale to be 1
    latent_regularization = torch.mean(torch.pow(rotation, 2)) +  1e2 * torch.mean(torch.pow(translation, 2)) + 1e2 * torch.mean(torch.pow(scale - 1, 2))
