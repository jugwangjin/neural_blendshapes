...
def main(args, device, dataset_train2, dataset_train):
    ...
    brows_coeffs = ['browDown_L', 'browDown_R', 'browInnerUp_L', 'browInnerUp_R', 'browOuterUp_L', 'browOuterUp_R']
    mouths_coeffs = ['mouthClose', 'mouthDimple_L', 'mouthDimple_R', 'mouthFrown_L', 'mouthFrown_R', 'mouthFunnel', 'mouthLeft', 'mouthLowerDown_L', 'mouthLowerDown_R', 'mouthPress_L', 'mouthPress_R', 'mouthPucker', 'mouthRight', 'mouthRollLower', 'mouthRollUpper', 'mouthShrugLower', 'mouthShrugUpper', 'mouthSmile_L', 'mouthSmile_R', 'mouthStretch_L', 'mouthStretch_R', 'mouthUpperUp_L', 'mouthUpperUp_R']
...
# 
    #     # also save the gt 
        ...
        features[:, [ict_facekit.expression_names.tolist().index(name) for name in brows_coeffs]] *= 2
        ...
        file_name = os.path.join(output_dir, f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_exag_brows_twice.png')
        save_manipulation_image(pred_color_masked, views_subset, gbuffers["normal"], gbuffer_mask, file_name)

    #     # determine output image name. 
        shading_name = f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_exag_brows_twice.png'
        save_shading(pred_color_masked, cbuffers, gbuffers, views_subset, Path(output_dir), 0, ict_facekit=ict_facekit, save_name=shading_name, transparency=True)

        features = return_dict['features'].clone().detach()
        
        features[:, [ict_facekit.expression_names.tolist().index(name) for name in mouths_coeffs]] *= 2
        features = features.clamp(0, 1)
        # features[:, :53] = views_subset['mp_blendshape'][:, neural_blendshapes.encoder.ict_facekit.mediapipe_to_ict]
        return_dict = neural_blendshapes(features=features)
        deformed_vertices = return_dict['expression_mesh_posed']
        ...
        file_name = os.path.join(output_dir, f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_exag_mouth_twice.png')
        save_manipulation_image(pred_color_masked, views_subset, gbuffers["normal"], gbuffer_mask, file_name)

    #     # determine output image name. 
        shading_name = f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_exag_mouth_twice.png'
        save_shading(pred_color_masked, cbuffers, gbuffers, views_subset, Path(output_dir), 0, ict_facekit=ict_facekit, save_name=shading_name, transparency=True)
        if iter_ > 300:
            break# 
