...
def main(args):
    ...
    # lmks = mesh_points_by_barycentric_coordinates(FLAMEServer.v_template.cpu().data.numpy(), 
    #                                                 FLAMEServer.faces_tensor.cpu().data.numpy(), lmk_face_idx, lmk_b_coords)

    # print(lmks)
    # write it to a file
    # with open('flame_lmks.txt', 'w') as f:
        # for i /in range(len(lmks)):
            # f.write(f'lmk {i+1}: {lmks[i]}\n')
    # exit()
    ...
    # camera_pose[:3, 3] = [0, 0, 4]  # Set camera position
    # scene.add(camera, pose=camera_pose)

    # # Create a light
    # light = pyrender.SpotLight(color=np.ones(3), intensity=3.0)
    # light_pose = np.eye(4)
    # light_pose[:3, 3] = [0, 1, 2]  # Set light position
    ...
    for i in range(optimized_flame_expressions.size(0)):
        ...
        # ict_mesh_node = pyrender.Node(pyrender.Mesh.from_trimesh(ict_mesh))
        # flame_mesh_node = pyrender.Node(pyrender.Mesh.from_trimesh(flame_mesh))

        # ict_mesh_node2 = pyrender.Node(pyrender.Mesh.from_trimesh(ict_mesh))
        # flame_mesh_node2 = pyrender.Node(pyrender.Mesh.from_trimesh(flame_mesh))

        # scene.add_node(ict_mesh_node)
        # scene.add_node(flame_mesh_node)
        # # Set the pose of the mesh node
        # pose = np.eye(4)
        # # move the mesh node for 1 unit to the y-axis
        # pose[:3, 3] = [-1, 1, 0]
        # scene.set_pose(ict_mesh_node, pose)
        # pose[:3, 3] = [1, 1, 0]
        # scene.set_pose(flame_mesh_node, pose)

        # scene.add_node(ict_mesh_node2)
        # scene.add_node(flame_mesh_node2)
        # # Set the pose of the mesh node
        # pose = np.eye(4)
        # # rotate the mesh node 90 degrees to the y-axis
        # pose[:3, :3] = np.array([[0, 0, 1], [0, 1, 0], [-1, 0, 0]])
        # # move the mesh node for -1 unit to the y-axis
        
        # # pose[:3, 3] = [i * 2, 0, 0]  # Set the position of the mesh node
        # pose[:3, 3] = [-1, -1, 0]
        # scene.set_pose(ict_mesh_node2, pose)
        # pose[:3, 3] = [1, -1, 0]
        # scene.set_pose(flame_mesh_node2, pose)
        ...
        # pyrender.write_png(f'{mesh_save_dir}/ict_exp_{i}_rendered_image.png', color)

        # scene.remove_node(ict_mesh_node)
        # scene.remove_node(flame_mesh_node)
        # scene.remove_node(ict_mesh_node2)
        # scene.remove_node(flame_mesh_node2)



    # rotated_mesh = torch.einsum('bni, bji -> bnj', mesh[None], R)

    # mesh = (s * rotated_mesh + T)[0]

    # # save mesh as obj using open3d
    # mesh_o3d = o3d.geometry.TriangleMesh()
    # mesh_o3d.vertices = o3d.utility.Vector3dVector(mesh.cpu().data.numpy())
    # mesh_o3d.triangles = o3d.utility.Vector3iVector(ICTmodel.faces.cpu().data.numpy())
    # o3d.io.write_triangle_mesh('ict_canonical.obj', mesh_o3d)

    # # save mesh of flame
    # mesh_o3d = o3d.geometry.TriangleMesh()
    # mesh_o3d.vertices = o3d.utility.Vector3dVector(flame_neutral_verts[0].cpu().data.numpy())
    # mesh_o3d.triangles = o3d.utility.Vector3iVector(FLAMEServer.faces_tensor.cpu().data.numpy())
    # o3d.io.write_triangle_mesh('flame_canonical.obj', mesh_o3d)

    # print(ict_identity)

    # # print('flame lmks', flame_neutral_lmks)
    # # print('ict lmks', ict_lmk)
