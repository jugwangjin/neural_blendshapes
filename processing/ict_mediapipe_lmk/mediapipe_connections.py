"""MediaPipe FaceMesh contour edges (mediapipe.jpg style)."""

# lips, eyes, brows, face oval (from mediapipe face_mesh_connections.FACEMESH_CONTOURS)
FACEMESH_LIPS = (
...
FACEMESH_CONTOURS = (
    ...
    + FACEMESH_RIGHT_EYEBROW
    + FACEMESH_FACE_OVAL
    + tuple(_ring_edges(LEFT_IRIS_MP))
    + tuple(_ring_edges(RIGHT_IRIS_MP))
)


def connections_for_landmarks(mp_indices):
    mp_set = set(int(i) for i in mp_indices)
    return [(a, b) for a, b in FACEMESH_CONTOURS if a in mp_set and b in mp_set]
