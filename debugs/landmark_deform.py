...
def deform_soft(V, F, b, bc, w):
    ...
    jj = b
    I = scipy.sparse.coo_matrix((data, (ii, jj)), shape=[b.shape[0], C.shape[1]]).tocsr()
    W = scipy.sparse.diags(w)
    
    A = scipy.sparse.vstack([C, W@I])
...
if __name__ == '__main__':
    ...
    # get jawopen mesh 
    jawOpen_index = ict_facekit.expression_names.tolist().index('jawOpen')
    expression = torch.zeros(1, ict_facekit.num_expression)
    expression[0, ict_facekit.jaw_index] = 1
    jawopen_mesh = ict_facekit.forward(expression_weights=expression, identity_weights=ict_facekit.identity, to_canonical=True)

    # save using open3d
    jawopen_o3d = o3d.geometry.TriangleMesh()
    print(jawopen_mesh[0].cpu().data.numpy().shape, ict_facekit.faces.cpu().data.numpy().shape)
    jawopen_o3d.vertices = o3d.utility.Vector3dVector(jawopen_mesh[0].cpu().data.numpy())
    jawopen_o3d.triangles = o3d.utility.Vector3iVector(ict_facekit.faces.cpu().data.numpy().astype(np.int32))

    o3d.io.write_triangle_mesh('debug/landmark_deform_jawopen.obj', jawopen_o3d)


    jawopen_F = ict_facekit.faces.cpu().data.numpy().astype(np.int32)
    jawopen_V = jawopen_mesh[0].cpu().data.numpy()

    b = np.array(ict_facekit.landmark_indices, dtype=np.int32)

    # get eye look right mesh
    eyeLookOut_L_index = ict_facekit.expression_names.tolist().index('eyeLookOut_L')
    eyeLookOut_R_index = ict_facekit.expression_names.tolist().index('eyeLookOut_R')
    expression = torch.zeros(1, ict_facekit.num_expression)
    expression[0, eyeLookOut_R_index] = 1
    expression[0, eyeLookOut_L_index] = 1
    eyeLookOut_mesh = ict_facekit.forward(expression_weights=expression, identity_weights=ict_facekit.identity, to_canonical=True)

    # save using open3d
    eyeLookOut_o3d = o3d.geometry.TriangleMesh()
    eyeLookOut_o3d.vertices = o3d.utility.Vector3dVector(eyeLookOut_mesh[0].cpu().data.numpy())
    eyeLookOut_o3d.triangles = o3d.utility.Vector3iVector(ict_facekit.faces.cpu().data.numpy().astype(np.int32))

    o3d.io.write_triangle_mesh('debug/landmark_deform_eyeLookOut.obj', eyeLookOut_o3d)

    eyeLookOut_bc = eyeLookOut_mesh[0].cpu().data.numpy()[b]

    ws = [0.5, 1., 2.]

    for w in ws:
        w = w * np.ones_like(b)
        deformed_V = deform_soft(jawopen_V, jawopen_F, b, eyeLookOut_bc, w)
        
        # save using open3d
        deformed_o3d = o3d.geometry.TriangleMesh()
        deformed_o3d.vertices = o3d.utility.Vector3dVector(deformed_V)
        deformed_o3d.triangles = o3d.utility.Vector3iVector(jawopen_F.astype(np.int32))

        o3d.io.write_triangle_mesh(f'debug/landmark_deform_deformed_{w[0]}.obj', deformed_o3d)