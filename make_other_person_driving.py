...
def main(args, device, dataset_train2, dataset_train):
    ...
    model_path_2 = os.path.join(args.output_dir_2, args.run_name_2, 'stage_1', 'network_weights', 'neural_blendshapes_latest.pt')
    ...
    neural_blendshapes_2 = get_neural_blendshapes(model_path=model_path_2, train=args.train_deformer, ict_facekit=ict_facekit, aabb = ict_mesh_aabb, face_normals=face_normals,device=device)

    # shader
    lgt = light.create_env_rnd()    
    try:
        shader = NeuralShader.load(os.path.join(args.output_dir_2, args.run_name_2, 'stage_1', 'network_weights', 'shader_latest.pt'), device=device)
    except:
        shader = NeuralShader.load(os.path.join(args.output_dir_2, args.run_name_2, 'stage_1', 'network_weights', 'shader.pt'), device=device)
...
# 
    #     # also save the gt 
        ...
        return_dict = neural_blendshapes_2(features=features)
        deformed_vertices = return_dict['expression_mesh_posed']
        d_normals = ict_canonical_mesh.fetch_all_normals(deformed_vertices, ict_canonical_mesh)
        ...
        file_name = os.path.join(output_dir, f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_driving.png')
        save_manipulation_image(pred_color_masked, views_subset, gbuffers["normal"], gbuffer_mask, file_name)

    #     # determine output image name. 
        shading_name = f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_driving.png'
        save_shading(pred_color_masked, cbuffers, gbuffers, views_subset, Path(output_dir), 0, ict_facekit=ict_facekit, save_name=shading_name, transparency=True)
        ...
        file_name = os.path.join(output_dir, f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_drivingnoper.png')
        save_manipulation_image(pred_color_masked, views_subset, gbuffers["normal"], gbuffer_mask, file_name)

    #     # determine output image name. 
        shading_name = f'{args.run_name}_{args.video_name}_{views_subset["idx"][0]}_drivingnoper.png'
        save_shading(pred_color_masked, cbuffers, gbuffers, views_subset, Path(output_dir), 0, ict_facekit=ict_facekit, save_name=shading_name, transparency=True)
...
if __name__ == '__main__':
    ...
    args2.output_dir_2 = args.output_dir_2
    args2.run_name_2 = args.model_name_2
