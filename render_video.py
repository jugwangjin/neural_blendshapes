...
def main(args, device, dataset_train2, dataset_train):
    ...
    for iter_, views_subset in progress_bar:
        ...
        print(file_name)
        save_manipulation_image(pred_color_masked, views_subset, gbuffers["normal"], gbuffer_mask, file_name, normal=False)
...
if __name__ == '__main__':
    ...
    dataset_train    = DatasetLoader(args2, train_dir=args.train_dir, sample_ratio=5, pre_load=False, train=True)
    dataset_val      = DatasetLoader(args2, train_dir=args.eval_dir, sample_ratio=5, pre_load=False)
